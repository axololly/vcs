...
use crate::core::{action::{Action, ActionHistory}, graph::Graph, hash::ObjectHash, snapshot::Snapshot, stash::Stash, trash::{Entry, Trash, TrashStatus}, user::{User, Users}};
...
impl Repository {
    ...
    pub fn set_current_user(&mut self, username: &str) -> Result<()> {
        if !self.users.has_user(username) {
            bail!("no user with name {username:?} exists in the repository.");
        }
